...
下表是对上述集中数据库在指定机型是上操作2000调简单数据的对比结果:

|DB/Item|SQLite|WCDB|ORMLite|GreenDao|Realm|
|---|----|----|---|----|----|
|版本|-|1.0.2|5.0|3.2.2|3.4.0|
|insert|4805ms|4753ms|200ms|80ms|40ms|
|query(2000条数据)|20.4ms|11ms|23.2ms|4.6ms|0.2ms|
|体积(arm下so)|-| 900KB|396KB|140KB|1.7M(仅 armeabi-v7a so 体积)|
|优缺点|- |sqlite相同|运行时反射效率低|使用稍微复杂，编译时注解效率高|1.体积大；2不支持id自增长；3不支持线程切换;4不支持limit|